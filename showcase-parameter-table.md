...
- `SHOWCASE_GLOBAL_SCALE = 1.2`
- 代码位置：`frontend/composables/useShowcaseRail.ts`
- 作用范围：背景层图片、前景层黑色遮罩、前景层镂空窗格会一起按这个比例同步放大或缩小。

| # | 参数 | 当前值 / 表达式 | 代码位置 | 调整说明 |
|---|---|---|---|---|
| 1 | 背景层图片大小尺寸参数 | `frameHeightPx = scale( clamp(viewportHeight * 0.34, 220, 380) )`；`frameWidthPx = scale( clamp(baseFrameHeightPx * 1.28, 280, 520) )` | `frontend/composables/useShowcaseRail.ts:87-94` | 这是背景层每张图的基础尺寸来源。当前还会再乘上全局比例 `SHOWCASE_GLOBAL_SCALE = 1.2`。 |
...
| 6 | 前景层镂空窗格大小尺寸参数 | `holeWidthPx = frameWidthPx`；`holeHeightPx = frameHeightPx` | `frontend/composables/useShowcaseRail.ts:108-109` | 这是洞口本身的尺寸来源。由于 `frameWidthPx / frameHeightPx` 已经包含全局比例，所以洞口会和图片一起同步放大缩小。 |
...
| 7 | 前景层黑色遮罩大小尺寸参数 | `matteWidthPx = frameWidthPx + mattePadXPx * 2`；`matteHeightPx = frameHeightPx + mattePadYPx * 2` | `frontend/composables/useShowcaseRail.ts:90-95,106-107` | 遮罩外框尺寸由图片尺寸再加上下左右 padding 得来。当前 `mattePadXPx / mattePadYPx` 也会一起乘上全局比例。 |
...
- 想调背景图整体大小：先看 `frontend/composables/useShowcaseRail.ts:15,87-97`
- 想调前景洞口大小：先看 `frontend/composables/useShowcaseRail.ts:108-109`
- 想调黑色遮罩外框大小：先看 `frontend/composables/useShowcaseRail.ts:91-97,106-107`
