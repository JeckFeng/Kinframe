...
### 本地Ollama配置和deepseek配置
本地宿主机已经安装了Ollama,使用宿主机的ollama即可，无需重复安装
Ollama中已经部署了视觉大模型：qwen3-vl:8b；
deepseek API key 为："sk-01c26a5b243e43dcb6f508a17ac4ecf3";
deepseek 使用的模型名称为："deepseek-v4-flash"
...
2. Ollama 视觉分析。
   ...
   - 步骤 2：输出图像主体、场景、情绪、主题色、天气、室内、室外、可能的图片分类。
...
4. 失败兜底。
   - 步骤 1：Ollama 失败时（设置超时时间为500s）保留原分类或 fallback 分类。
   - 步骤 2：DeepSeek 失败时（设置超时时间为500s）使用 fallback design。
...
### 注意事项：
管理员手动覆盖 final_caption 后，普通用户后续修改 user_message 不得自动清除管理员覆盖。
只有管理员显式执行“恢复自动文案”操作，才重新按 user_message > ai_caption > null 计算 final_caption。
